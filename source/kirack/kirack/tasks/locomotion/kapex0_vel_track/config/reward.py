...
def feet_gait(
    ...
    if command_name is not None:
        ...
        # lin_vel(xy) + ang_vel(z) 가중 합 — ang_vel 명령 범위가 좁으므로 가중치 ↑
        # ang_vel 0.1 rad/s 만으로도 cmd_score > 0.1 발화하도록
        cmd_score = torch.norm(cmd[:, :2], dim=1) + 2.0 * torch.abs(cmd[:, 2])
        reward *= cmd_score > 0.1
