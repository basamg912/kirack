...
def torso_com_levels(
    ...
    timeout_idx = env.termination_manager._term_name_to_term_idx["time_out"]
    height_idx = env.termination_manager._term_name_to_term_idx["base_height"]
    orient_idx = env.termination_manager._term_name_to_term_idx["bad_orientation"]

    timeout_ratio = env.termination_manager._last_episode_dones[:, timeout_idx].float().mean().item()
    height_fail_ratio = env.termination_manager._last_episode_dones[:, height_idx].float().mean().item()
    orient_fail_ratio = env.termination_manager._last_episode_dones[:, orient_idx].float().mean().item()

    # 에피소드 경계에서만 업데이트
    if env.common_step_counter % env.max_episode_length == 0:
        # termination_ratio 가 threshold 이하면 난이도 상승 (range 확장)
        if (
            timeout_ratio > threshold
            # and height_fail_ratio < max_failure_ratio
            # and orient_fail_ratio < max_failure_ratio
        ):
